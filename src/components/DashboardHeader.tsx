import { MessageCircle, BookOpen, Lightbulb, Settings, Globe, User } from "lucide-react";
    ...
    <header className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
      ...
      <div className="flex items-center gap-3 animate-fade-in-up-delay-1">
        {quickActions.map((action) => (
          <button
            key={action.label}
            className={`${action.bg} flex items-center gap-2 px-4 py-2.5 rounded-2xl text-sm font-semibold text-foreground transition-all hover:scale-105 hover:shadow-md active:scale-95`}
          >
